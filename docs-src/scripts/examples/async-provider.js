...
async function loadConfig() {
  ...
  // Change the value below to see the difference in execution time
  await sleep(100);
  return {
    host: "localhost",
    port: 5432,
...
const databaseProvider = {
  async registerAsync(container) {
    // Configure database settings
    container.set("dbConfig", await loadConfig());
...
await container.registerAsync(databaseProvider);
